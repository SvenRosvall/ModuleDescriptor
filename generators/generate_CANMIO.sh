...
do
  cat <<EOF
    "channel$ch" : "Channel $ch"$(commaIf $ch != $channels)
EOF
done
cat <<EOF
  },
...
do
    cat <<EOF
        {"value": $((4+$ch*4)), "overload":{"nv": $((9+$ch*7)), "labels": [
              {"value": 0, "label": "\${channel$ch} - Input Changed"},
              {"value": 1, "label": "\${channel$ch} - Output Changed"},
              {"value": 2, "label": "\${channel$ch} - Reached OFF"},
              {"value": 3, "label": "\${channel$ch} - Output Changed"},
              {"value": 4, "label": "\${channel$ch} - AT1"}$( commaIf -n "$hasAnalogue" )
EOF

    if [ -n "$hasAnalogue" ]
    then
        cat <<EOF
              {"value": 5, "label": "\${channel$ch} - Threshold"},
              {"value": 6, "label": "\${channel$ch} - Lower Threshold"}
EOF
    fi
    cat <<EOF
            ]
        ...
        {"value": $((5+$ch*4)), "overload":{"nv": $((9+$ch*7)), "labels": [
              {"value": 0, "label": "\${channel$ch} - TWO_ON"},
              {"value": 2, "label": "\${channel$ch} - Reached MID"},
              {"value": 4, "label": "\${channel$ch} - AT2"}$( commaIf -n "$hasAnalogue" )
EOF
    if [ -n "$hasAnalogue" ]
    then
        cat <<EOF
              {"value": 6, "label": "\${channel$ch} - Upper Threshold"}
EOF
    fi
    cat <<EOF
            ]
        ...
        {"value": $((6+$ch*4)), "overload":{"nv": $((9+$ch*7)), "labels": [
              {"value": 2, "label": "\${channel$ch} - Reached ON"},
              {"value": 4, "label": "\${channel$ch} - AT3"}
            ]
          }
        },
        {"value": $((7+$ch*4)), "overload":{"nv": $((9+$ch*7)), "labels": [
              {"value": 4, "label": "\${channel$ch} - AT4"}
            ]
          }
        }$( commaIf $ch != $channels)
EOF
...
do
    cat <<EOF
            {"value": $((3+$ch*5)), "overload":{"nv": $((9+$ch*7)), "labels": [
                  {"value": 1, "label": "\${channel$ch} - Change"},
                  {"value": 2, "label": "\${channel$ch} - Change"},
                  {"value": 3, "label": "\${channel$ch} - Change"},
                  {"value": 4, "label": "\${channel$ch} - AT1"}
                ]
              }
            },
            {"value": $((4+$ch*5)), "overload":{"nv": $((9+$ch*7)), "labels": [
                  {"value": 1, "label": "\${channel$ch} - ON"},
                  {"value": 2, "label": "\${channel$ch} - ON"},
                  {"value": 3, "label": "\${channel$ch} - ON"},
                  {"value": 4, "label": "\${channel$ch} - AT2"}
                ]
              }
            },
            {"value": $((5+$ch*5)), "overload":{"nv": $((9+$ch*7)), "labels": [
                  {"value": 1, "label": "\${channel$ch} - OFF"},
                  {"value": 2, "label": "\${channel$ch} - OFF"},
                  {"value": 3, "label": "\${channel$ch} - OFF"},
                  {"value": 4, "label": "\${channel$ch} - AT3"}
                ]
              }
            },
            {"value": $((6+$ch*5)), "overload":{"nv": $((9+$ch*7)), "labels": [
                  {"value": 1, "label": "\${channel$ch} - FLASH"},
                  {"value": 4, "label": "\${channel$ch} - AT4"}
                ]
              }
            },
